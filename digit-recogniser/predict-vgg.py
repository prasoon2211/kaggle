...
tune = False
train_new = True
wts_filepath = "wts-best-vgg.hdf5"
...
if train_new:
    ...
    clf.fit(x_train, y_train, epochs=20, batch_size=32, validation_split=0.2, callbacks=[checkpoint])
    clf = make_model(load=True)
else:
    # load best model
    clf = make_model(load=True)
...
pred_df.to_csv("pred-vgg.csv", index=False)
